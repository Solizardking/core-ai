...
usage() {
  ...
  echo ""
  echo "Install the Clawd Core x DFlow trading skill for Clawd Code."
  echo ""
  echo "Options:"
  ...
  echo ""
  echo "Examples:"
  echo "  ./install.sh              # Install to ~/.clawd/skills/clawd-dflow/"
  echo "  ./install.sh --project    # Install to ./.clawd/skills/clawd-dflow/"
  echo "  ./install.sh --path /tmp  # Install to /tmp/clawd-dflow/"
}
...
echo "Clawd Core x DFlow skill installed to $TARGET ($MODE)"
echo ""
echo "Next steps:"
echo "  1. Install the Clawd MCP server (if not already):"
echo "     npx clawd-mcp@latest  # configure in .clawd/settings.json or your MCP client"
echo ""
