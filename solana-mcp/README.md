...
`server/cloudrun.ts` listens on `PORT` and defaults to `8080`, so local MCP clients should use `http://localhost:8080/mcp`. Set `PORT=3000 pnpm dev:local` if you want to match the inspector script's `http://localhost:3000/mcp` target.
