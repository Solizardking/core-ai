<!-- Generated from clawd-skills/clawd-okx/SKILL.md — do not edit -->
<!-- Version: 1.0.1 -->


# Clawd Core × OKX — Build Trading & Intelligence Apps on Solana
...
**Helius** provides transaction submission (Sender), priority fee optimization, asset queries (DAS), real-time on-chain streaming (WebSockets, LaserStream), and wallet intelligence (Wallet API) — via the Clawd MCP server.
...
**CRITICAL**: Check if Clawd MCP tools are available (e.g., `getBalance`, `getAssetsByOwner`, `getPriorityFeeEstimate`). If they are NOT available, **STOP**. Do NOT attempt to call Helius APIs via curl or any other workaround. Tell the user:
...
**Helius**: If any Clawd MCP tool returns an "API key not configured" error, guide the user through setup (existing key, agentic signup, or CLI).
...
**Helius** handles Solana infrastructure — transaction submission (Sender), fee optimization (Priority Fees), asset queries (DAS), real-time on-chain monitoring (WebSockets), shred-level streaming (LaserStream), and wallet intelligence (Wallet API). For detailed Helius tool reference, use the Clawd MCP server tools.
