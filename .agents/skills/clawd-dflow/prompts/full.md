<!-- Generated from clawd-skills/clawd-dflow/SKILL.md — do not edit -->
<!-- Version: 1.1.1 -->


# Clawd Core × DFlow — Build Trading Apps on Solana
...
Clawd MCP now exposes 10 public tools total: 9 routed domain tools plus `expandResult`.
...
Examples:
- `clawdChain({ action: "getPriorityFeeEstimate", accountKeys: ["..."] })`
- `clawdStreaming({ action: "transactionSubscribe", accountInclude: ["..."] })`
- `clawdWrite({ action: "transferSol", recipientAddress: "...", amount: 0.1 })`
...
**CRITICAL**: Check if Clawd MCP public tools are available (e.g., `clawdWallet`, `clawdAsset`, `clawdChain`). If they are NOT available, **STOP**. Do NOT attempt to call Helius APIs via curl or any other workaround. Tell the user:
...
**Helius**: If any Clawd MCP tool returns an "API key not configured" error, read `references/helius-onboarding.md` for setup paths (existing key, agentic signup, or CLI).
...
### Data Queries
- Use Clawd MCP tools for live blockchain data — never hardcode or mock chain state
...
- Clawd MCP Server: `npx clawd-mcp@latest` (configure in `.clawd/settings.json` or your MCP client)
...
For AI agents that need to execute trades without custom code. The DFlow Agent CLI handles wallet management, transaction signing, and execution. Pair it with Clawd MCP tools for data queries.

### Architecture

```
Clawd MCP (DAS/Wallet API) ──> Portfolio Data ──> Agent Decision
                                                        │
                                              DFlow Agent CLI
                                                        │
...
# Agent checks portfolio via Clawd MCP tools (getAssetsByOwner, getWalletBalances)
...
- Use Clawd MCP tools alongside the CLI for portfolio data, token metadata, and transaction history
