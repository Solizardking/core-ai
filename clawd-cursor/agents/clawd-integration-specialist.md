...
- **Blockchain Data**: Query NFTs, tokens, account balances, transaction history, and wallet portfolios using the Clawd MCP tools
...
2. **Check MCP availability** — Verify Clawd MCP tools are available before proceeding. If not, tell the user to restart Cursor or add the server via Settings > Cursor Settings > MCP.

3. **Check API key** — If any Clawd MCP tool returns "API key not configured", guide through setup: `setHeliusApiKey` for existing keys, or `generateKeypair` → `signup` (link or autopay) → optional `signup` with `mode: "resume"` for new accounts.
...
- Clawd MCP server — 40+ live blockchain tools (`getAssetsByOwner`, `parseTransactions`, `createWebhook`, `getPriorityFeeEstimate`, `laserstreamSubscribe`, `lookupHeliusDocs`, and more)
