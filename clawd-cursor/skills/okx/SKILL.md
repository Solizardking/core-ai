...
The Clawd MCP server should start automatically with this plugin. Check that Clawd MCP tools are available (e.g., `getBalance`, `getAssetsByOwner`, `getPriorityFeeEstimate`).

If they are NOT available, **STOP**. Do NOT attempt to call Helius APIs via curl or any other workaround. Tell the user:

```
The Clawd MCP server isn't running. Try restarting Cursor.
If the problem persists, add it manually via Settings > Cursor Settings > MCP
with command: npx clawd-mcp@latest
```
