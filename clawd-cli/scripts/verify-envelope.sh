...
# into the clawd-cli package.
#
# Usage: bash clawd-cli/scripts/verify-envelope.sh [pubkey]
#
...
CLI="node dist/bin/clawd-cli.js"
...
if [ ! -f dist/bin/clawd-cli.js ]; then
  echo "dist/bin/clawd-cli.js not found — run pnpm build first." >&2
  exit 1
fi
