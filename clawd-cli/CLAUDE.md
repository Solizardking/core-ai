...
```
bin/clawd-cli.ts          # Entry point — Commander.js program, registers all commands
src/
  ...
  lib/
    ...
    config.ts          # ~/.clawd/config.json — jwt, apiKey, network, projectId
...
3. `~/.clawd/config.json` → `apiKey` field
...
pnpm install
pnpm build          # tsc → dist/
pnpm dev            # tsx bin/clawd-cli.ts (runs directly)
node dist/bin/clawd-cli.js --help
```
...
5. Register in `bin/clawd-cli.ts` with Commander.js
