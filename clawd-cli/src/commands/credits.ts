...
/**
 * Phase 2 — `clawd-cli credits` to top up prepaid credits on an agent-plan
...
async function runPayWithStored(
  ...
  if (stored.payerWallet && payerAddress !== stored.payerWallet) {
    exitWithError(
      "INVALID_INPUT",
      `Local keypair wallet (${payerAddress}) does not match the wallet that created this credits intent (${stored.payerWallet}). Run \`clawd-cli credits --restart\` to start over.`,
      undefined,
      !!options.json,
    );
  }
...
async function runResume(
  ...
  if (!stored) {
    ...
    exitWithError(
      ...
      "No pending credits purchase. Run `clawd-cli credits` first.",
      undefined,
      !!options.json,
    );
  }
...
function emitPaymentRequired(
  ...
  console.log();
  console.log(chalk.gray("Once paid, run `clawd-cli credits --resume` to confirm locally."));
}
...
function emitPending(
  ...
  console.log(chalk.gray(`\nRun \`clawd-cli credits --resume\` again in a moment.`));
}
