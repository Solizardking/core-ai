...
const NPM_REGISTRY_URL = "https://registry.npmjs.org/clawd-cli/latest";
...
function getUpdateCommand(pm: string): string {
  switch (pm) {
    case "pnpm":  return "pnpm add -g clawd-cli@latest";
    case "volta":  return "volta install clawd-cli@latest";
    case "bun":    return "bun add -g clawd-cli@latest";
    default:       return "npm install -g clawd-cli@latest";
  }
}
  ...
  try {
    ...
    if (cmp >= 0) {
      ...
      if (options.json) {
        ...
        console.log(chalk.green(`\n  clawd-cli is up to date (v${VERSION})`));
      }
      return;
    }
    ...
    if (options.check) {
      ...
      if (options.json) {
        ...
        console.log(`\n  Run ${chalk.cyan("clawd-cli update")} to install the latest version.`);
      }
      return;
    }
    ...
    spinner?.succeed(`Updated clawd-cli v${VERSION} → v${latest}`);
