...
export async function whoamiCommand(options: WhoamiOptions): Promise<void> {
  ...
  try {
    ...
    if (!jwt) {
      ...
      console.log(
        chalk.gray(
          "  • Run `clawd-cli login` if you have an existing Helius account (browser-based, OAuth/PKCE).",
        ),
      );
      console.log(
        chalk.gray("  • Run `clawd-cli signup` to create a new account (keypair + USDC)."),
      );
      if (apiKey) {
        console.log(
          chalk.gray(
    ...
    if (serverVerified === true) {
      ...
      console.log(chalk.red("\n✖ Backend rejected the token — run `clawd-cli login` to re-authenticate."));
    }
    if (expired) {
      console.log(chalk.yellow("\n⚠ Session has expired. Run `clawd-cli login` to refresh."));
    }
  } catch (error) {
