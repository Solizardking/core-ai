...
export async function signupCommand(options: SignupOptions): Promise<void> {
  ...
  try {
    ...
    // Top-level JWT is set so subsequent commands (clawd-cli login, etc.) reuse it,
...
async function runPayWithStored(
  ...
  // rotated `id.json` between `clawd-cli signup` (link) and `clawd-cli signup --pay`,
  ...
  if (payerAddress !== stored.walletAddress) {
    exitWithError(
      "INVALID_INPUT",
      `Local keypair wallet (${payerAddress}) does not match the wallet that created this payment intent (${stored.walletAddress}). Run \`clawd-cli signup --restart\` to start over.`,
      undefined,
      !!options.json,
    );
  }
...
async function runResume(
  ...
  if (!stored) {
    ...
    exitWithError(
      ...
      "No pending signup found in config. Run `clawd-cli signup` first.",
      undefined,
      !!options.json,
    );
  }
...
function emitPaymentRequired(
  ...
  console.log();
  console.log(chalk.gray(`Once paid, run \`clawd-cli signup --resume\` to finish setup.`));
}
...
function emitUpgradeRequired(
  ...
  const message = `Wallet is already on plan "${result.currentPlan}". Plan changes go through \`clawd-cli upgrade\`.`;
  if (options.json) {
    outputJson({
...
function emitPending(
  ...
  console.log(chalk.gray(`\nRun \`clawd-cli signup --resume\` again in a moment.`));
}
...
function emitExpired(stored: PendingSignup, options: SignupOptions): void {
  ...
  console.error(chalk.gray("Run `clawd-cli signup` to start a fresh signup."));
  process.exit(ExitCode.GENERAL_ERROR);
}
