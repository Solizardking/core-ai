...
/**
 * Phase 2 — `clawd-cli upgrade`. Mirrors `clawd-cli signup` with three modes:
...
async function runPayWithStored(
  ...
  if (stored.payerWallet && payerAddress !== stored.payerWallet) {
    exitWithError(
      "INVALID_INPUT",
      `Local keypair wallet (${payerAddress}) does not match the wallet that created this upgrade intent (${stored.payerWallet}). Run \`clawd-cli upgrade --restart\` to start over.`,
      undefined,
      !!options.json,
    );
  }
...
async function runResume(
  ...
  if (!stored) {
    ...
    exitWithError(
      ...
      "No pending upgrade found in config. Run `clawd-cli upgrade --plan ...` first.",
      undefined,
      !!options.json,
    );
  }
...
function emitPaymentRequired(
  ...
  console.log();
  console.log(chalk.gray("Once paid, run `clawd-cli upgrade --resume` to confirm locally."));
}
...
function emitPending(
  ...
  console.log(chalk.gray(`\nRun \`clawd-cli upgrade --resume\` again in a moment.`));
}
...
function emitExpired(stored: PendingUpgrade, options: UpgradeOptions): void {
  ...
  console.error(chalk.gray("Run `clawd-cli upgrade --plan ...` to start fresh."));
