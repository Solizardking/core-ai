...
/**
 * Phase 2 — `clawd-cli pay <paymentIntentId>` for renewal flows.
 *
 * Modes:
...
export async function payCommand(
  ...
  try {
    ...
    console.log(
      chalk.gray(
        `Once paid, run \`clawd-cli pay ${paymentIntentId} --resume\` to confirm locally.`,
      ),
    );
  } catch (error) {
...
function emitPending(
  ...
  console.log(
    chalk.gray(
      `\nRun \`clawd-cli pay ${paymentIntentId} --resume\` again in a moment.`,
    ),
  );
}
