...
export async function statusCommand(options: OutputOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (!jwt) {
      ...
      console.log(chalk.red("Not logged in. Run `clawd-cli login` or `clawd-cli signup` first."));
    ...
    if (projects.length === 0) {
      ...
      console.log(chalk.gray("or run `clawd-cli signup` if you don't have an account yet."));
    ...
    if (usage) {
      ...
      if (parseFloat(pctRemaining) < 20) {
        console.log(chalk.yellow(`\n  Warning: Less than 20% of credits remaining. Run \`clawd-cli upgrade\` to see upgrade options.`));
      }
    }
