...
export const CLI_GUIDANCE: Record<string, string> = {
  INVALID_API_KEY: 'Run `clawd-cli config set-api-key <key>` with a valid key, or `clawd-cli usage` to check your current plan and credits.',
  RATE_LIMITED: 'Run `clawd-cli usage` to check remaining credits. Back off and retry, or upgrade your plan for higher limits.',
  NO_API_KEY: 'Run `clawd-cli config set-api-key <key>` or set HELIUS_API_KEY env var. Get a key at https://dashboard.helius.dev.',
  ...
  NOT_LOGGED_IN: 'Run `clawd-cli login` to authenticate, or `clawd-cli signup` to create a new account.',
  KEYPAIR_NOT_FOUND: 'Run `clawd-cli keygen` to generate a keypair first.',
  NO_PROJECTS: 'Create a project in the dashboard at https://dashboard.helius.dev, or run `clawd-cli signup` if you do not have an account yet.',
  MULTIPLE_PROJECTS: 'Specify a project ID. Run `clawd-cli projects` to list them.',
  PROJECT_NOT_FOUND: 'Run `clawd-cli projects` to list available projects.',
  NO_API_KEYS: 'Run `clawd-cli apikeys create` to create an API key.',
  INVALID_INPUT: 'Check command usage with --help.',
  AUTH_FAILED: 'Your credentials are invalid or expired. Run `clawd-cli login` to re-authenticate.',
  PROJECT_EXISTS: 'A project with this name already exists. Run `clawd-cli projects` to list them or pick a different name.',
