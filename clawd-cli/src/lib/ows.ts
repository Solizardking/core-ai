/**
 * Open Wallet Standard (OWS) integration for clawd-cli.
 *
 * Provides helpers to detect the `ows` CLI, list wallets, and extract
 * Solana addresses.  Used by `clawd-cli wallet ows-link` and OWS-aware
 * command flags.
 *
 * NOTE: The CLI helpers (isOwsInstalled, getOwsSolanaAddress) are mirrored in
 * clawd-mcp/src/utils/ows.ts.  Keep the two copies in sync when changing
