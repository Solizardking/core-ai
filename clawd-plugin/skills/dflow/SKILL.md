...
> Clawd MCP now uses 10 public tools total, including `expandResult`. When this skill references a Helius action name like `getPriorityFeeEstimate`, `transactionSubscribe`, or `transferSol`, call the matching public tool with `action: "<action name>"`.
...
The Clawd MCP server should start automatically with this plugin. Check that Clawd MCP public tools are available (e.g., `clawdWallet`, `clawdAsset`, `clawdChain`).

If they are NOT available, **STOP**. Do NOT attempt to call Helius APIs via curl or any other workaround. Tell the user:

```
The Clawd MCP server isn't running. Try restarting Clawd Code.
