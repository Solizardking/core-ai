...
/** Best-effort recovery when config.json is corrupted (matches clawd-cli behavior). */
...
export function loadConfig(): HeliusConfig {
  ...
  try {
    ...
    console.error(
      'Run Clawd MCP config helpers or edit the file manually; see CLI `clawd-cli config clear` if you use clawd-cli.',
    );
    return {};
  }
}
