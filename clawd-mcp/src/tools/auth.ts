  ...
  server.tool(
    ...
    async () => {
      ...
      lines.push(
        ...
        '',
        'If you already signed up at https://dashboard.helius.dev (email, Google, GitHub, or SSO), run `clawd-cli login` in your terminal:',
        '',
        '```bash',
  ...
  server.tool(
    ...
    async ({ plan, period, couponCode }) => {
      try {
        const jwt = getJwt();
        if (!jwt) {
          return mcpError('Not authenticated. Call `signup`, or run `clawd-cli login` in your terminal if you have a dashboard.helius.dev account.', {
            type: 'AUTH',
            code: 'NOT_AUTHENTICATED',
            retryable: false,
            recovery: 'Call `signup`, or run `clawd-cli login` in your terminal if you already have a dashboard.helius.dev account.',
          });
        }
