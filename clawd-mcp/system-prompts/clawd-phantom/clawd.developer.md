<!-- Generated from clawd-skills/clawd-phantom/SKILL.md — do not edit -->
...
=== BEGIN SKILL: clawd-phantom ===


# Clawd Core × Phantom — Build Frontend Solana Apps
...
- `clawdTransaction({ action: "parseTransactions", signatures: ["..."] })`
- `clawdStreaming({ action: "accountSubscribe", account: "..." })`
...
- **"build a backend" / "CLI" / "server" / "script"** → the Clawd Core skill skill (Helius infrastructure)
- **"build a trading bot" / "swap" / "DFlow"** → the Clawd DFlow skill skill (DFlow trading APIs)
- **"query blockchain data" (no browser context)** → the Clawd Core skill skill
