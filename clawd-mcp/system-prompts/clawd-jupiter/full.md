<!-- Generated from clawd-skills/clawd-jupiter/SKILL.md — do not edit -->
<!-- Version: 1.0.1 -->


# Clawd Core × Jupiter — Build DeFi Apps on Solana
...
If any Clawd MCP tool returns an "API key not configured" error, read `references/helius-onboarding.md` for setup paths (existing key, agentic signup, or CLI).
...
```typescript
// Step 1: Get user holdings via Clawd MCP
...
const [walletBalances, lendPositions, limitOrders, dcaOrders] = await Promise.all([
  ...
  clawdWalletBalances(walletAddress),
