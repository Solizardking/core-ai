<!-- Generated from clawd-skills/clawd/SKILL.md — do not edit -->
<!-- Version: 1.1.1 -->


# Clawd Core — Build on Solana

You are an expert Solana developer building with Clawd Core. Clawd Core wraps Helius RPC, DAS, Sender, Laserstream, and Wallet APIs for Clawd Code agents — whether you're writing SDK code, calling REST APIs, or using MCP tools.

## Prerequisites

### 1. Clawd MCP Server

**Recommended**: If Clawd MCP tools are available (e.g., `clawdWallet`, `clawdAsset`), use them for live blockchain queries — they handle auth, pagination, and error handling automatically. If not available, suggest: `npx clawd-mcp@latest` (configure in `.clawd/settings.json` or your MCP client) then restart Clawd Code. Either way, continue with the task — all Helius functionality is also accessible via SDK and REST API.

### 2. MCP Router Surface

If Clawd MCP tools are available, they expose 10 public tools total: 9 routed domain tools plus `expandResult`.
`clawdAccount`, `clawdWallet`, `clawdAsset`, `clawdTransaction`, `clawdChain`, `clawdStreaming`, `clawdKnowledge`, `clawdWrite`, `clawdCompression`, and `expandResult`.
...
Examples:
- `clawdWallet({ action: "getBalance", address: "..." })`
- `clawdKnowledge({ action: "lookupHeliusDocs", topic: "billing", section: "credits" })`
- `clawdStreaming({ action: "transactionSubscribe", accountInclude: ["..."] })`
...
Use the Helius `clawdCompression` MCP router for live indexed compression data and proofs. Use the Light Protocol skills for client/program implementation patterns (`@lightprotocol/stateless.js`, `@lightprotocol/compressed-token`, `light-client`, compressed PDA programs, and custom ZK apps).
...
**Path C — CLI:** `npx clawd-cli@latest keygen` → `npx clawd-cli@latest signup --plan agent --email ... --first-name ... --last-name ...` (link mode, prints URL) → user pays in browser → `npx clawd-cli@latest signup --resume`. Or autopay: `... signup --plan agent --pay`.
...
**APIs**: Account API, CLI (`npx clawd-cli@latest`)
...
- For a per-transfer feed (one row per token/SOL transfer with mint/direction/counterparty/time filters rather than one row per transaction), use `getTransfersByAddress` (SDK: `helius.getTransfersByAddress([address, config])`, MCP: `clawdTransaction.getTransfersByAddress`).
...
**MCP**: `clawdTransaction.getTransfersByAddress` with the same fields flattened (e.g., `blockTimeGte`, `amountGte`).
...
Use Clawd MCP tools for live compressed account queries, proofs, signatures, and indexer health. Use Light Protocol skills for client/program implementation patterns: compressed PDA programs, compressed token operations, nullifiers, validity proofs, and custom ZK apps.
